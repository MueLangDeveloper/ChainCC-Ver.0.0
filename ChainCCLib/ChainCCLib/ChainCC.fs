module ChainCCRunner
    ...
    type ChainCC(_code: string) =
        ...
        member val private ref: OrderedDictionary<string, string> List = List<OrderedDictionary<string, string>>() with get, set
        // as Queue
        member val private stack: DynamicStack List = List<DynamicStack>() with get, set
        // as Queue
        member val private vars: OrderedDictionary<string, Dynamic> List = List<OrderedDictionary<string, Dynamic>>() with get, set
        // as Queue
        member val result: string = "" with get, set
        member val cmdLock: LockedCode List = List<LockedCode>() with get, set
        member val grammars: OrderedDictionary<string, string> = OrderedDictionary<string, string>() with get, set
        ...
        member private ccc.DivideGroupAndInvoke (_code: string) =
                ...
                | s when appendFlg && s.StartsWith ".after " ->
                    ...
                    | s when s[0] = '(' ->
                        ...
                        while metaCode.Count = 0 || Regex.Replace(metaCode.Aggregate(fun x y -> x + y), @"""(\\.|[^""])*""", "")
                            .Where(fun c -> match c with | '(' | ')' -> true | _ -> false).Count() % 2 <> 0 do
        ...
        member ccc.DividedGroupInvoke(codes: string List)(opt: ChainCCOption): string =
            ...
            if ccc.cmdLock[0].First <> "" then
                ccc.cmdLock.Insert(0, LockedCode())
                result <- result + ccc.DividedGroupInvoke (ccc.cmdLock[1].First.Split('\n').ToList()) opt
                ccc.cmdLock.RemoveAt 0
            ...
            if codes[0].StartsWith "lock" = false then
                for _code in codes do
                    if ifzero = false then
                        ...
                        | c when c.StartsWith "match " ->
                            if ccc.success || first then
                                ...
                                if regex.Success = false then ccc.success <- false
                                    ...
                                    ccc.ref.[0][name] <- ccc.ref.[0][name] + regex.Value
                        ...
                        | c when c.StartsWith "if " ->
                            ...
                            | "value" ->
                                if ccc.stack[0].Pop() = reverse then ifzero <- true
                            | _ -> raise(Exception())
                        ...
                        | c when c.StartsWith "result " ->
                            ...
                            match code with
                            | ide when ide.StartsWith "first" -> ccc.result <- $"{ccc.stack[0].Pop()}" + ccc.result
                            | ide when ide.StartsWith "last" -> ccc.result <- ccc.result + $"{ccc.stack[0].Pop()}"
                            | _ -> raise(Exception())
                        | c when c.StartsWith "delete " ->
                            code <- code[7..].Trim ' '
                            ccc.ref[0].RemoveAt(ccc.ref[0].Select(fun kv i -> [|kv.Key; $"{i}"|]).First(fun array -> array[0] = code)[1] |> int)
                        ...
                        | c when c.StartsWith "call " ->
                            ccc.ref.Insert(0, OrderedDictionary<string, string>())
                            ccc.stack.Insert(0, DynamicStack())
                            ccc.vars.Insert(0, OrderedDictionary<string, Dynamic>())
                            code <- code[5..].Trim ' '
                            result <- result + ccc.DivideGroupAndInvoke ccc.grammars[code]
                            ccc.ref.RemoveAt 0
                            ccc.stack.RemoveAt 0
                            ccc.vars.RemoveAt 0
                        | c when c.StartsWith '(' ->
                            result <- result + ccc.DivideGroupAndInvoke code[1 .. (code.Length - 1)]
                        | _ -> raise(Exception())
                        if opt.Trim then ccc.parseStr <- String.Join("", ccc.parseStr.SkipWhile(fun c1 -> ccc.trims.Any(fun c2 -> c1 = c2)))

                    if ccc.isDebug then
                        if ccc.stack[0].Count <> 0 then
                            printf "stk: [ %A" (ccc.stack[0].Stack.Reverse().ToArray()[0])
                            for v in ccc.stack[0].Stack.Reverse().ToArray()[1..] do
                                printf ", %A" v
                            printfn " ]"
                        else printfn "stk: [  ]"
                        if ccc.ref[0].Count <> 0 then
                            printf "ref: [ %A" (ccc.ref[0].ToArray()[0])
                            for v in ccc.ref[0].ToArray()[1..] do
                                printf ", %A" v
                            printfn " ]"
                        else printfn "ref: [  ]"
                ...
                if ccc.cmdLock[0].Last <> "" then
                    ccc.cmdLock.Insert(0, LockedCode())
                    result <- result + ccc.DividedGroupInvoke (ccc.cmdLock[1].Last.Split('\n').ToList()) opt
                    ccc.cmdLock.RemoveAt 0
            else
            ...
            if loopFlg && ccc.success then
                result <- result + ccc.DividedGroupInvoke codes opt
                ccc.success <- true
            if refFlg && ccc.success = false then ccc.ref.[0].RemoveAt(ccc.ref.[0].Select(fun kv i -> [|kv.Key; $"{i}"|]).First(fun t -> t.[0] = name).[1] |> int)
